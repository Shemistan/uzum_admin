...
func Prepare(ctx context.Context, db DBTX) (*Queries, error) {
	...
	if q.getBasketsStmt, err = db.PrepareContext(ctx, GetBaskets); err != nil {
		return nil, fmt.Errorf("error preparing query GetBaskets: %w", err)
	}
...
func (q *Queries) Close() error {
	...
	if q.getBasketsStmt != nil {
		if cerr := q.getBasketsStmt.Close(); cerr != nil {
			err = fmt.Errorf("error closing getBasketsStmt: %w", cerr)
		}
	}
...
func (q *Queries) WithTx(tx *sql.Tx) *Queries {
	return &Queries{
		...
		getBasketsStmt:    q.getBasketsStmt,
